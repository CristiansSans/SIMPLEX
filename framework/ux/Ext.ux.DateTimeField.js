...
Ext.ux.DateTime = function(config) {
	...
	Ext.extend(Ext.ux.DateTime, Ext.form.Field, {
			/**//**
				 ...
				 *      Let superclass to create hidden field instead of
			...
			/**//**
				 * @cfg {Number} timeWidth Width of time field in pixels
				 *      (defaults to 100)
				 */
			,
			timeWidth : 100
			...
			dtSeparator : ' '
			/**//**
				 * @cfg {String} hiddenFormat Format of datetime used to store
				 *      value in hidden field and submitted to server (defaults
				 *      to 'Y-m-d H:i:s' that is mysql format)
				 */
			,
			...
			otherToNow : true
			/**//**
				 * @cfg {Boolean} emptyToNow Set field value to now if on
				 *      attempt to set empty value. If it is true then
				 *      setValue() sets value of field to current date and time
				 *      (defaults to false) /**
				 * @cfg {String} timePosition Where the time field should be
				 *      rendered. 'right' is suitable for forms and 'bellow' is
			...
			timePosition : 'right' // valid values:'bellow', 'right'
				/**//**
					 * @cfg {String} dateFormat Format of DateField. Can be
					 *      localized. (defaults to 'm/y/d')
					 */
				,
				dateFormat : 'm/d/y'
				/**//**
					 * @cfg {String} timeFormat Format of TimeField. Can be
					 *      localized. (defaults to 'g:i A')
					 */
				,
				timeFormat : 'g:i A'
				/**//**
					 * @cfg {Object} dateConfig Config for DateField
					 *      constructor.
					 */
				/**//**
					 * @cfg {Object} timeConfig Config for TimeField
					 *      constructor.
					 */
				...
				// relay events
				this.relayEvents(this.df, ['focus', 'specialkey', 'invalid',
						'valid']);
				this.relayEvents(this.tf, ['focus', 'specialkey', 'invalid',
						'valid']);
			...
			initDateValue : function() {
				this.dateValue = this.otherToNow ? new Date() : new Date(1970,
						0, 1, 0, 0, 0);
			}
			// }}}
			// {{{
			...
			// {{{
			/**//**
				 * @return {Boolean} true = valid, false = invalid private Calls
				 *         isValid methods of underlying DateField and TimeField
			...
			// {{{
			/**//**
				 * private Just to prevent blur event when clicked in the middle
				 * of fields
				 */
			,
			...
			// {{{
			/**//**
				 * @return {Boolean} true = valid, false = invalid callse
...
// register xtype
Ext.reg('capturactiva.ux.Generic.DateTime', Ext.ux.DateTime);